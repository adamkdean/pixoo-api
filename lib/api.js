...
export class PixooAPI {
  ...
  drawTextCenter(text, y, color = Color.White) {
    ...
    const chars = [...text]
    chars.map((c, i) => this.drawChar(c, [x + i * 4, y], color))
  }

  // @y: horizontal line to draw text on (from top pixel)
  // @color: [r, g, b]
  drawTextLeft(text, y, color = Color.White) {
    const chars = [...text]
    chars.map((c, i) => this.drawChar(c, [i * 4, y], color))
  }

  // @y: horizontal line to draw text on (from top pixel)
  // @color: [r, g, b]
  drawTextRight(text, y, color = Color.White) {
    const x = this.size - text.length * 4 + 1
    const chars = [...text]
    chars.map((c, i) => this.drawChar(c, [x + i * 4, y], color))
  }
  ...
  //   let imageData
  //   if (isPNG) {
  //     imageData = await resizedImage.raw().toBuffer()
  //   } else {
  //     imageData = await resizedImage.jpeg().toBuffer()
  //   }
