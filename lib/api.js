...
export class PixooAPI {
  ...
  drawTextCenter(text, y, color = Color.White) {
    const x = Math.floor((this.size - text.length * 4) / 2)
    const chars = [...text]
    chars.map((c, i) => this.drawChar(c, [x + i * 4, y], this.parseColor(color)))
  }

  // @y: horizontal line to draw text on (from top pixel)
  // @color: [r, g, b]
  // @padding: number of pixels to pad the text on the left and right sides
  drawTextLeft(text, y, color = Color.White, padding = 0) {
    const chars = [...text]
    chars.map((c, i) => this.drawChar(c, [padding + i * 4, y], this.parseColor(color)))
  }

  // @y: horizontal line to draw text on (from top pixel)
  // @color: [r, g, b]
  // @padding: number of pixels to pad the text on the left and right sides
  drawTextRight(text, y, color = Color.White, padding = 0) {
    const x = this.size - text.length * 4 - padding + 1
    const chars = [...text]
    chars.map((c, i) => this.drawChar(c, [x + i * 4, y], this.parseColor(color)))
  }
  ...
  drawText(text, pos, color = Color.White) {
    [...text].map((c, i) => this.drawChar(c, [i * 4 + pos[0], pos[1]], this.parseColor(color)))
  }
